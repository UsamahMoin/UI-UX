...
function BalanceChart({ model, period, onPeriodChange }: { model: ReturnType<typeof balanceChartModel>; period: BalancePeriod; onPeriodChange: (period: BalancePeriod) => void }) {
  ...
  return <div className={`sg-chart sg-trend-${trend}`}>
    ...
    <div className="sg-chart-controls"><div className="sg-chart-scrub-label"><span>Inspect a day</span><span>← → arrow keys</span></div><input type="range" min="0" max={points.length - 1} value={selectedIndex} onChange={e => setSelected(Number(e.target.value))} aria-label="Explore daily balance" aria-valuetext={`${fullDate(point.date)}: ${money(point.balance)}`} />
    ...
    <details className="sg-chart-table"><summary>View daily balances as a table</summary><div tabIndex={0} role="region" aria-label="Daily balance records"><table><caption>{period} daily balances</caption><thead><tr><th scope="col">Date</th><th scope="col">Balance</th></tr></thead><tbody>{points.map(item => <tr key={item.date}><th scope="row">{fullDate(item.date)}</th><td>{money(item.balance)}</td></tr>)}</tbody></table></div></details>
  </div>;
}
