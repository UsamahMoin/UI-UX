...
export function ProjectThumbnail({ slug, name }: { slug: string; name: string }) {
  return <div className={`project-cover cover-${slug}`}>
    {slug === 'meridian' && <><Image unoptimized width={1024} height={1536} src={sitePath('/images/meridian-poster.png')} alt="" loading="lazy"/><h3>MIDNIGHT<small>ON MERIDIAN</small></h3><span className="cover-caption">A CITY OUT OF TIME.</span></>}
