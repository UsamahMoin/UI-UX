...
import { meridianCast, meridianScreenings, meridianSynopsis } from '@/lib/meridian-film';
  ...
  return <div className="meridian-film">
    <div className={`meridian-film-sheet ${person ? 'is-cast-page' : ''}`}>
      <section className="meridian-film-art" aria-label={person ? `${person.first} ${person.last}, fictional cast profile` : 'Midnight on Meridian film poster'}>
        ...
        <div className="meridian-film-title"><span>TWENTY YEARS MISSING. TWELVE MINUTES LEFT.</span><h2>MIDNIGHT<small>ON MERIDIAN</small></h2><p>A supernatural thriller</p></div>
        {person ? <article className="meridian-cast-story"><span>{person.credit}</span><h3>{person.first}<br/><span>{person.last}</span></h3><div className="meridian-cast-role">AS {person.role.toUpperCase()}</div><h4>{person.line}</h4><p>{person.description}</p><p className="meridian-character-detail">{person.note}</p><a href={sitePath('/work/meridian/experience')}><ArrowLeft/> Back to the film</a><small>Fictional cast profile · original film concept</small></article> : <div className="meridian-film-tagline"><p>The last tram was declared empty.<br/><span>Ada can hear the passengers.</span></p><a href="#meridian-screenings">Find a screening <ArrowDown/></a></div>}
      </section>
      <div className="meridian-film-billing">{!person && <p className="meridian-film-synopsis">{meridianSynopsis}</p>}<p>MERIDIAN PICTURE HOUSE PRESENTS<br/><strong>MIRA ELLERY · JONAH REED · INES CALDER</strong><br/>IN <b>“MIDNIGHT ON MERIDIAN”</b><br/>AN ORIGINAL FILM CONCEPT BY <strong>USAMAH MOIN</strong></p><div><span>FICTIONAL FILM &amp; CAST · AI-ASSISTED ARTWORK</span></div></div>
